# Code from https://github.com/godatadriven/pydantic-avro
...
class AvroTypeConverter:
    ...
    def _get_avro_type(self, field_props: dict, avro_type_dict: dict) -> dict:
        ...
        if (
            "prefixItems" in field_props
            or ("minItems" in field_props and "maxItems" in field_props)
        ) and t == "array":
            t = "tuple"
    ...
    def _tuple_to_avro(self, field_props: dict, avro_type_dict: dict) -> dict:
        ...
        if not prefix_items:
            ...
            if not prefix_items:
                raise ValueError(
                    f"Tuple Field '{field_props}' does not have any items ."
                )
        possible_types = []
        for prefix_item in prefix_items:
            ...
            if not item_type:
                raise ValueError(
                    f"Field '{avro_type_dict}' does not have a defined type."
                )
...
class AvroBase(BaseModel):
    ...
    def avro_schema(
        ...
        """
        schema = (
            cls.model_json_schema(by_alias=by_alias)
            if PYDANTIC_V2
            else cls.schema(by_alias=by_alias)
        )
    ...
    @staticmethod
    def _avro_schema(
        schema: dict, namespace: str, avro_type_handler: AvroTypeConverter
    ) -> dict:
